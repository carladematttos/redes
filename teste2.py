...
def receive_udp_message(port, timeout=5):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind(("0.0.0.0", port))
    sock.settimeout(timeout)  # Define um tempo limite para receber mensagens (em segundos)
...
def main():
    ...
    for port in ports:
        for seq_number, matricula in zip(seq_numbers, matriculas):
            ...
            if response is None:
                break  # Sai do loop se nenhuma mensagem for recebida dentro do tempo limite
